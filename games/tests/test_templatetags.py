...
class GameExtrasTemplateTagTest(ViewTestCase):
    ...
    def test_game_opponent(self):
        template_to_render = Template(
            "{% load game_extras %}" "{% game_opponent game %}"
        )
    ...
    def test_game_coin(self):
        template_to_render = Template("{% load game_extras %}" "{% game_coin game %}")
        rendered_template = template_to_render.render(self.context)
        # test that template tag loads correctly
        self.assertInHTML(
            "bg-warning",
    ...
    def test_game_badge(self):
        template_to_render = Template("{% load game_extras %}" "{% game_badge game %}")
        rendered_template = template_to_render.render(self.context)
        # test that template tag loads correctly
        self.assertInHTML(
    ...
    def test_game_detail_title(self):
        template_to_render = Template(
            "{% load game_extras %}" "{% game_detail_title game %}"
        )
        rendered_template = template_to_render.render(self.context)
        # test that template tag loads correctly
        self.assertInHTML(
