...
from games.models import Coin, Game
from games.views import (
    GameCheckRedirectView,
    GameCoinRedirectView,
    GameCreateView,
    GameListView,
)
...
class GameCreateViewTest(ViewTestCase):
    def test_get_form_kwargs(self):
        ...
        self.assertEqual(
            self.user, kwargs.get("user"), msg="user has been placed into the kwargs"
        )
...
class GameCoinRedirectViewTest(ViewTestCase):
    ...
    def test_get_redirect_url(self):
        ...
        self.assertEqual(
            self.game.status,
            Game.Status.PLAYER_2,
            msg="Game updated to be next players turn",
        )
        self.assertTrue(
            Coin.objects.filter(
                game=self.game, player=self.user, column=self.column, row=0
            ).exists(),
        ...
        self.assertEqual(
            ...
            msg="redirect to the game detail view",
        )
